...
이 프로젝트는 역할에 따라 파일을 분리하여 체계적으로 관리하기 위해 다음과 같이 구성했습니다.

-   `app/`: 실제 웹 서버가 사용자에게 보여줄 파일(HTML, CSS 등)이 위치하는 소스 코드 디렉토리입니다.
-   `images/`: `README.md` 문서에 사용될 스크린샷 등 이미지 파일을 보관하는 디렉토리입니다.
-   `Dockerfile`: 웹 서버 구축을 위한 위한 Docker 이미지 설계도입니다. 프로젝트의 루트에 두어 이 프로젝트 전체가 Docker로 관리된다는 것을 명확히 합니다.
-   `README.md`: 프로젝트에 대한 설명, 실행 방법, 학습내용을 기록하는 문서입니다.
...
### 컨테이너와 포트 매핑
컨테이너는 분리된 환경에서 실행됩니다. 마치 저만의 IP 주소를 가진 독립된 미니 컴퓨터와 같습니다. 따라서 제 진짜 컴퓨터와는 네트워크가 분리되어 있어, 외부에서 컨테이너 내부의 포트로 직접 접근할 수 없습니다.
이런 격리된 컨테이너의 특정 포트를 외부에 노출시키기 위해 포트매핑이 필요합니다. 이것은 제 컴퓨터의 특정 포트를 컨테이너의 포트와 연결하는 통로를 만들어주는 작업입니다.

### 절대 경로와 상대 경로
/로 시작하는 절대경로와 현재위치를 기준으로 하는 상대경로가 있습니다.
절대경로는 위치가 명확하지만 다른 컴퓨터에는 해당 경로가 없을 확률이 높기에 명령이 불가합니다.
상대경로는 폴더를 통째로 옮겨도 명령어를 실행하는 위치를 정확히 알면 동일하게 작동합니다.

### 이미지와 컨테이너의 차이점.
이미지는 프로젝트를 실행할 수 있도록 도와주는 설계도와 같습니다.
컨테이너는 이미지(설계도)를 기반으로 실제로 실행하는 구간입니다.
이미지 : Dockerfile로 빌드(Build)하고, 한 번 만들면 변경할 수 없습니다.
컨테이너 : 이미지를 실행(Run)한 결과물이며, 동작 중에 변경할 수 있지만 컨테이너가 삭제되면 그 변경 내용은 사라집니다.
...
### 1. “호스트 포트가 이미 사용 중”일 때
포트가 이미 사용중이라면, sudo lsof -i :(포트번호) 를 사용해 어떤 프로세스가 포트를 사용하는지 확인합니다.
이후 프로세스가 불필요하다면 종료를, 종료할 수 없다면 docker run 의 포트를 변경합니다.(8080:80 > 8081.80)

### 2. 컨테이너 삭제 후 데이터 유실 방지
컨테이너 삭제 시 내부 데이터도 같이 사라집니다. 이를 방지라혀면 컨테이너 외부에 데이터를 저장해야합니다.
docker volume create 로 볼륨을 만들고 -v volume1:app(예시) 로 연결합니다.

### 3. 어려웠던 점.
터미널의 파일 경로를 햇갈렸습니다. pwd명령어를 몰랐을 때에는 경로가 햇갈려 엉뚱한곳에 파일을 생성하거나 지웠습니다. 지금은 pwd명령어로 작업하고 있는 데이터의 경로를 수시로 확인하여 경로를 햇갈리지 않도록 예방하고 있습니다.
git push를 처음 진행할 때에 비밀번호 입력이 토큰 입력인 줄 모르고 계속 비밀번호를 입력했습니다. 네이토에게 문제를 물어봐 토큰을 생성하고 push를 정상적으로 진행했습니다.
